...
def makeRun(
  ...
  print(
      f"makeRun called with RUN_ID={RUN_ID}, MODEL_ID={MODEL_ID},"
      f" MODEL_TEMPERATURE={MODEL_TEMPERATURE},"
      f" MAX_NORMS_PER_5K={MAX_NORMS_PER_5K},"
      f" MAX_CHAR_BUFFER={MAX_CHAR_BUFFER},"
      f" EXTRACTION_PASSES={EXTRACTION_PASSES},"
      f" INPUT_PROMPTFILE={INPUT_PROMPTFILE},"
      f" INPUT_GLOSSARYFILE={INPUT_GLOSSARYFILE},"
      f" INPUT_EXAMPLESFILE={INPUT_EXAMPLESFILE},"
      f" INPUT_SEMANTCSFILE={INPUT_SEMANTCSFILE},"
      f" INPUT_TEACHFILE={INPUT_TEACHFILE}"
  )
  ...
  USE_OPENROUTER = os.getenv("USE_OPENROUTER", "1").lower() in {
      "1",
      "true",
      "yes",
  }
  ...
  if USE_OPENROUTER:
    if not OPENROUTER_KEY:
      print(
          "WARNING: OPENROUTER (OPENAI_API_KEY) key not set – OpenRouter call"
          " will fail.",
          file=sys.stderr,
      )
  else:
    if not GOOGLE_API_KEY:
      print(
          "WARNING: GOOGLE_API_KEY not set – direct Gemini call will likely"
          " fail.",
          file=sys.stderr,
      )
  ...
  GLOSSARY_FILE = (
      Path(INPUT_GLOSSARYFILE)
      if INPUT_GLOSSARYFILE
      else (run_dir / "glossary.json")
  )
  ...
  try:
    MAX_CHAR_BUFFER = (
        int(MAX_CHAR_BUFFER) if MAX_CHAR_BUFFER is not None else 5000
    )
  except Exception:
    MAX_CHAR_BUFFER = 5000
  try:
    EXTRACTION_PASSES = (
        int(EXTRACTION_PASSES) if EXTRACTION_PASSES is not None else 2
    )
  except Exception:
    EXTRACTION_PASSES = 1
  ...
  if input_override:
    ...
    preferred = [
        ...
        and not p.name.endswith("_output.json")
    ]
    fallback = [p for p in files if not p.name.endswith("_output.json")]
  ...
  if not PROMPT_FILE.exists():
    print(
        f"[WARN] Prompt file missing at {PROMPT_FILE}; using minimal default"
        " prompt.",
        file=sys.stderr,
    )
    PROMPT_DESCRIPTION = (
        "Extract Norms, Tags, and Parameters. Return a JSON object with an"
        " 'extractions' array."
    )
  else:
  ...
  if not INPUT_FILE or not INPUT_FILE.exists():
    print(
        f"[WARN] Input file missing at {INPUT_FILE}; proceeding with empty"
        " input text.",
        file=sys.stderr,
    )
  ...
  if TEACH_MODE:
    ...
    if ENTITY_SEMANTICS_FILE.exists():
      PROMPT_DESCRIPTION += "\n\n" + ENTITY_SEMANTICS_FILE.read_text(
          encoding="utf-8"
      )
  ...
  def _load_examples_from_py(py_path: Path) -> List[Any]:
    if not py_path or not py_path.exists():
      print(
          f"[WARN] Examples file not found at {py_path}. Using empty examples.",
          file=sys.stderr,
      )
      return []
    try:
      ...
      if not spec or not spec.loader:
        print(
            f"[WARN] Could not create module spec for examples at {py_path}",
            file=sys.stderr,
        )
        return []
      ...
      if isinstance(ex, list):
        return ex
      print(
          f"[WARN] No list named EXAMPLES found in {py_path}.", file=sys.stderr
      )
      return []
    except Exception as e:
      print(
          f"[WARN] Failed to load EXAMPLES from {py_path}: {e}", file=sys.stderr
      )
      return []
  ...
  print(
      f"[INFO] Loaded {len(EXAMPLES)} few-shot examples from {_examples_path}"
  )
  ...
  if not INPUT_FILE or not INPUT_FILE.exists():
    print(
        f"[WARN] Input file not found at {INPUT_FILE}. Using empty input.",
        file=sys.stderr,
    )
    INPUT_TEXT = ""
  else:
  ...
  extract_kwargs = dict(
      ...
      resolver_params={
          "fence_output": False,
          "format_type": lx.data.FormatType.JSON,
          "suppress_parse_errors_default": (
              True
          ),  ## TODODisable this for proper runs!!!
  ...
  if USE_OPENROUTER:
    ...
    lm_extra.update({
        ...
        ),  # forwarded to OpenAI-compatible client
        # Optional attribution headers (OpenRouter specific)
        "openrouter_referer": os.getenv("OPENROUTER_REFERER"),
        "openrouter_title": os.getenv(
            "OPENROUTER_TITLE", "LangExtract Rich Schema Runner"
        ),
    })
  else:
    # Direct Gemini path
  ...
  print(
      "[INFO] Using"
      f" {'OpenRouter' if USE_OPENROUTER else 'Direct Gemini'} provider with"
      f" model_id={MODEL_ID}"
  )
  ...
  def _call_and_capture(
    ...
    print(
        "[DEBUG] About to call lx.extract for chunk"
        f" {idx if idx is not None else 'single'}",
        file=sys.stderr,
    )
    print(
        f"[DEBUG] Extract kwargs keys: {list(extract_kwargs.keys())}",
        file=sys.stderr,
    )
    ...
    try:
      ...
      msg = (
          "[WARN] Extract failed for chunk"
          f" {idx if idx is not None else 'single'}: {safe_err}"
      )
      print(msg, file=sys.stderr)
      run_warnings.append(msg)
      # Synthesize a minimal valid rich object so the run completes
      synthesized = _synthesize_extraction(
          text, norms=[], errors=[safe_err], warnings=run_warnings
      )
      result = {"extractions": [synthesized]}
      raw_name = (
          f"chunk_{idx:03}.json" if idx is not None else "chunk_single.json"
      )
      try:
    ...
    print(
        "[DEBUG] Starting to save raw annotated document for chunk"
        f" {idx if idx is not None else 'single'}",
        file=sys.stderr,
    )
    try:
      raw_annotated_name = (
          f"raw_annotated_document_{idx:03}.json"
          if idx is not None
          else "raw_annotated_document_single.json"
      )
      ...
      raw_annotated_data["object_attributes"] = (
          [attr for attr in dir(annotated) if not attr.startswith("_")]
      ...
      try:
            ...
            if hasattr(annotated, attr_name):
              ...
              try:
                ...
                if attr_name == "extractions" and attr_value is not None:
                  try:
                    if hasattr(attr_value, "__iter__") and not isinstance(
                        attr_value, str
                    ):
                      first_few = []
                    ...
                    manual_extraction[attr_name]["sample_error"] = str(
                        sample_err
                    )
            else:
        ...
        serialization_attempts["manual_extraction"] = (
            f"error: {str(manual_err)}"
        )
      ...
      print(
          f"[DEBUG] Raw annotated document saved to: {raw_annotated_name}",
          file=sys.stderr,
      )

    except Exception as raw_annotated_err:
      print(
          f"[WARN] Failed to save raw annotated document: {raw_annotated_err}",
          file=sys.stderr,
      )

    # Save RAW resolver output for debugging (before any processing)
    print(
        "[DEBUG] Starting to save raw resolver output for chunk"
        f" {idx if idx is not None else 'single'}",
        file=sys.stderr,
    )
    try:
      raw_resolver_name = (
          f"raw_resolver_output_{idx:03}.json"
          if idx is not None
          else "raw_resolver_output_single.json"
      )
      ...
      print(
          f"[DEBUG] extractions attribute type: {type(extractions_attr)}",
          file=sys.stderr,
      )
      print(
          f"[DEBUG] extractions attribute is None: {extractions_attr is None}",
          file=sys.stderr,
      )
      ...
      try:
        ...
        if extractions is not None:
          raw_resolver_data["extractions_count"] = (
              len(extractions) if hasattr(extractions, "__len__") else "unknown"
          )
            ...
            for i, ext in enumerate(extractions):
              ...
              try:
                sample_extractions.append({
                    ...
                    "type": str(type(ext)),
                    "attributes": [
                        attr for attr in dir(ext) if not attr.startswith("_")
                    ],
                    "extraction_class": getattr(
                        ext, "extraction_class", "unknown"
                    ),
                    "extraction_text_preview": (
                        str(getattr(ext, "extraction_text", ""))[:200] + "..."
                        if len(str(getattr(ext, "extraction_text", ""))) > 200
                        else str(getattr(ext, "extraction_text", ""))
                    ),
                })
              except Exception as sample_err:
                sample_extractions.append(
                    {"index": i, "error": str(sample_err)}
                )
      ...
      print(
          f"[WARN] Failed to save raw resolver output: {raw_save_err}",
          file=sys.stderr,
      )
    ...
    try:
      ...
      for extraction_index, e in enumerate(extractions):
        ...
        item = {
            ...
            "alignment_status": _get_alignment_status_value(
                getattr(e, "alignment_status", None)
            ),
      ...
      derived_tags = extract_tags_from_norms(
          norms_to_process, tag_counter_start=1
      )
      derived_params = extract_parameters_from_norms(
          norms_to_process, param_counter_start=1
      )
      ...
      raw_name = (
          f"annotated_extractions_{idx:03}.json"
          if idx is not None
          else "annotated_extractions_single.json"
      )
      ...
      print(
          f"[WARN] Failed to persist raw annotated outputs: {pe}",
          file=sys.stderr,
      )
  ...
  print(
      "[INFO] Library-managed chunking enabled (max_char_buffer governs"
      " internal splits)"
  )
  _call_and_capture(INPUT_TEXT, None)
  print(
      f"[INFO] Raw annotated outputs saved to: {lx_output_dir}. Annotated-only"
      " mode complete."
  )
  return
