...
def makeRun(
  ...
  def _call_and_capture(
      ...
      for e in getattr(annotated, "extractions", []) or []:
        item = {
            ...
            "alignment_status": getattr(
                getattr(e, "alignment_status", None), "value", None
            ),
    ...
    def _question_signature(item: Dict[str, Any]) -> tuple:
      ...
      outs_norm = (
          tuple(sorted([_as_str(o).strip().lower() for o in outs]))
    ...
    def _parameter_signature(item: Dict[str, Any]) -> tuple:
      ...
      pth = (
          _as_str(item.get("parameter_path", item.get("tag_path", "")))
          .strip()
          .lower()
      )
    ...
    for e in legacy_extractions:
      ...
      if cls == "schema_version" and isinstance(parsed, str):
        ...
        meta["truncated"] = (
            bool(parsed) if not isinstance(parsed, bool) else parsed
        )
    ...
    print(
        f"[DEBUG] Aggregation counts — norms: pre={_norm_raw_count},"
        f" post_dedup={len(norms)}; tags: pre={_tag_raw_count},"
        f" post_dedup={len(tags)}; params: pre={_param_raw_count},"
        f" post_dedup={len(parameters)}"
    )
  ...
  if not all_extractions:
    ...
    synthesized = _synthesize_extraction(
        INPUT_TEXT, norms=[], errors=[err] + run_warnings, warnings=run_warnings
    )
  ...
  if not (
      isinstance(parsed_root, dict)
      and isinstance(parsed_root.get("extractions"), list)
      and parsed_root["extractions"]
  ):
    print(
        "[WARN] Model output missing 'extractions' non-empty list; synthesizing"
        " fallback."
    )
    parsed_root = {
        "extractions": [
            _synthesize_extraction(
                INPUT_TEXT,
                norms=[],
                errors=["Invalid root structure"],
  ...
  invalid_objects = [
      i
      for i, obj in enumerate(extractions_list)
      if not pp_schema.is_rich_schema(obj)
  ]
  if invalid_objects:
    print(
        "[WARN] One or more extraction objects missing required keys (indices:"
        f" {invalid_objects}). Will annotate errors and continue."
    )
    # Replace invalid objects with synthesized shells preserving index order
    for i in invalid_objects:
      parsed_root["extractions"][i] = _synthesize_extraction(
          INPUT_TEXT,
          norms=[],
  ...
  if schema_errors:
    primary.setdefault("quality", {}).setdefault("errors", []).extend(
        schema_errors
    )
  ...
  OUTPUT_FILE.write_text(
      json.dumps(parsed_root, ensure_ascii=False, indent=2), encoding="utf-8"
  )
  ...
  print(
      f"[INFO] Saved DSL glossary stub ({len(dsl_keys)} keys) → {GLOSSARY_FILE}"
  )
