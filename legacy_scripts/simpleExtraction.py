...
semantics_appendix = Path(
    "input_semanticsfiles/prompt_appendix_entity_semantics_min.md"
)
...
if not OPENROUTER_KEY:
  log(
      "[FATAL] OPENAI_API_KEY is not set. Add it to your .env or environment"
      " for OpenRouter usage."
  )
  raise RuntimeError(
      "OPENAI_API_KEY is not set. Add it to your .env or environment for"
      " OpenRouter usage."
  )

# Read local file contents as input text (UTF-8 with fallback)
file_path = Path(
    r"C:\Projects\Arqio\LangExtract\langextract\docs\test_docs\small_test_norms.txt"
)
try:
...
if _is_url(input_source):
  try:
    ...
    log(
        "[INFO] HEAD"
        f" status={h.status_code} content-type={h.headers.get('Content-Type','')} length={h.headers.get('Content-Length','?')}"
    )
  except Exception as e:
...
with ThreadPoolExecutor(max_workers=1) as ex:
  ...
  while True:
    try:
      ...
      raise
log(
    f"[INFO] Extraction completed in {time.time()-start:.1f}s — saving"
    " artifacts..."
)
