...
def _node_span_to_offsets(node: ast.AST, line_offsets: List[int]) -> Tuple[int, int]:
        ...
        def visit_Call(self, call: ast.Call) -> Any:
            ...
            for kw in call.keywords:
                if not isinstance(kw, ast.keyword):
                    continue
                if kw.arg in TargetKeys and isinstance(kw.value, ast.Constant) and isinstance(kw.value.value, str):
...
def translate_spanish_to_english(texts: List[str], config: TranslatorConfig) -> List[str]:
    ...
    if config.show_progress:
        print(
            f"Translating {len(texts)} field(s) in batches of {config.batch_size} using '{config.model_name}' (CPU)"
        )
...
def apply_replacements(source: str, spans: List[StringSpan], replacements: List[str]) -> str:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Translate ExampleData.text and Extraction.extraction_text string literals in a Python examples file.")
    ap.add_argument("input", type=Path, help="Input .py file (e.g., input_examplefiles/examples_enhanced.py)")
    ap.add_argument("output", type=Path, nargs="?", help="Output .py (default: *_en.py next to input)")
    ap.add_argument("--batch-size", type=int, default=64, help="Translation batch size")
    ap.add_argument("--model", default="Helsinki-NLP/opus-mt-es-en", help="Hugging Face model id")
    args = ap.parse_args()
    ...
    print(f"Found {len(texts)} string literal(s) to translate: keys in {sorted(TargetKeys)}")

    try:
        translated = translate_spanish_to_english(
            texts,
            TranslatorConfig(model_name=args.model, batch_size=args.batch_size, show_progress=True),
        )
