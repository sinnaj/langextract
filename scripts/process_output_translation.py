...
from scripts.translate_statement_text_mt import (
    TranslatorConfig,
    _collect_translation_targets,
    _set_by_path,
    translate_spanish_to_english,
)
from scripts.renumber_ids import (
    _walk_collect_ids,  # type: ignore
    _build_occurrence_counts,  # type: ignore
    _renumber_ids_in_place,  # type: ignore
    _rewrite_references,  # type: ignore
)


def _renumber_in_memory(data: Any, preserve_old_id: bool = False) -> tuple[Any, Dict[str, int], int, int]:
    ...
    _renumber_ids_in_place(work, counters, id_counts, unique_ref_map, preserve_old_id, per_prefix_totals)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Translate and renumber an extraction output JSON.")
    ap.add_argument("input", type=Path, help="Path to input output.json")
    ap.add_argument("--out-en", dest="out_en", type=Path, default=None, help="Path to write translated JSON (default: output_en.json next to input)")
    ap.add_argument("--out-ids", dest="out_ids", type=Path, default=None, help="Path to write renumbered JSON (default: output_ids.json next to input)")
    ap.add_argument("--also-notes", action="store_true", dest="also_notes", help="Also translate 'notes' fields")
    ap.add_argument("--batch-size", type=int, default=64, help="Batch size for translation (default: 64)")
    ap.add_argument("--model", default="Helsinki-NLP/opus-mt-es-en", help="Hugging Face model id (default: Helsinki-NLP/opus-mt-es-en)")
    ap.add_argument("--preserve-old-id", action="store_true", help="Preserve original IDs in an 'old_id' field during renumbering")
    args = ap.parse_args()
    ...
    out_en.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote translated JSON: {out_en}")

    # 2) Renumber IDs on the translated data
    new_data, per_prefix, total_objs, safe_refs = _renumber_in_memory(data, preserve_old_id=args.preserve_old_id)
    out_ids = args.out_ids or args.input.with_name("output_ids.json")
    out_ids.write_text(json.dumps(new_data, ensure_ascii=False, indent=2), encoding="utf-8")

    if total_objs:
        per_prefix_str = ", ".join(f"{k}={v}" for k, v in sorted(per_prefix.items()))
