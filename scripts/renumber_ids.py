...
def _renumber_ids_in_place(node: Any, counters: Dict[str, int], id_counts: Dict[str, int], unique_ref_map: Dict[str, str], preserve_old_id: bool, per_prefix_totals: Dict[str, int]) -> None:
    ...
    if isinstance(node, dict):
        ...
        for k, val in list(node.items()):
            ...
            if k in ("id", "old_id"):
                continue
            _renumber_ids_in_place(val, counters, id_counts, unique_ref_map, preserve_old_id, per_prefix_totals)
    elif isinstance(node, list):
        for it in node:
            _renumber_ids_in_place(it, counters, id_counts, unique_ref_map, preserve_old_id, per_prefix_totals)
    # primitives ignored
...
def _rewrite_references(node: Any, safe_map: Dict[str, str]) -> Any:
    ...
    if isinstance(node, list):
        return [safe_map.get(it, it) if isinstance(it, str) else _rewrite_references(it, safe_map) for it in node]
...
def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("Usage: renumber_ids.py <input.json> [<output.json>] [--preserve-old-id]")
        return 2
    inp = Path(argv[1])
    if not inp.exists():
    ...
    _renumber_ids_in_place(data, counters, id_counts, unique_ref_map, preserve_old_id, per_prefix_totals)
    ...
    per_prefix_str = ", ".join(f"{k}={v}" for k, v in sorted(per_prefix_totals.items()))
    # Report how many references were safely rewritten
    safe_refs = len(unique_ref_map)
    dup_ids = [k for k, c in id_counts.items() if c > 1]
    if dup_ids:
        print(f"Note: {len(dup_ids)} old ID value(s) were duplicated in input and could not be disambiguated for reference rewriting. Their references were left unchanged.")
