...
## Web Interface: Norms Sandbox

The LangExtract web interface includes a **Norms Sandbox** for interactively exploring and filtering extracted norms using feature-based criteria from Information Gain (IG) assessments.

### Features

- **Interactive Filtering**: Filter norms using features from `ig_assessment/tmp/ig.csv`
- **Tri-State Logic**: Uses Kleene three-valued logic (TRUE/FALSE/UNKNOWN) to evaluate which norms apply under selected conditions
- **Multiple Filter Types**:
  - Categorical features: Multi-select checkboxes
  - Numeric bins: Dropdown selectors
  - Integer features: Number inputs
- **Output Selection**: Switch between different extraction runs
- **Smart Filtering**: Norms that don't reference a filtered feature remain visible (evaluated as UNKNOWN)
...
2. Navigate to `http://localhost:5000` and select **Norms Sandbox**

3. Select filters from the right panel to see which norms remain applicable under those conditions

4. View detailed norm information by clicking "Details" on any norm card

### How It Works

The Sandbox uses the DSL evaluator from `ig_assessment/` to determine norm applicability:

- **TRUE**: Norm definitely applies under current filters (shown)
- **FALSE**: Norm cannot apply under current filters (hidden)
- **UNKNOWN**: Cannot determine if norm applies (shown - keeps norms that don't reference filtered features)

This allows you to explore which building code norms are relevant for specific project scenarios (e.g., educational buildings over 2000 m²).

### File Locations

- **Norms**: `output_runs/<timestamp>/enhanced_output/enhanced_extraction_results.json`
- **Features**: `ig_assessment/tmp/ig.csv` (generated by running `compute_ig.py`)
- **Latest Output**: Sandbox automatically selects the most recent output by default

## Norm Ingestion Pipeline with DNF Conversion

The repository includes a production-ready ingestion pipeline for storing extracted norms with boolean logic in PostgreSQL. This pipeline converts complex `applies_if`, `satisfied_if`, and `exempt_if` expressions to Disjunctive Normal Form (DNF) for efficient querying.

### Features

- **DNF Conversion**: Converts boolean expressions with AND, OR, NOT, and parentheses to OR-of-ANDs form
- **PostgreSQL Storage**: Normalized schema for norms, questions, clause groups, and requirements
- **Idempotent Ingestion**: Safe to re-run without duplicates
- **Expression Optimization**: Removes contradictions and deduplicates predicates
- **Type Inference**: Automatically classifies values as BOOLEAN, INTEGER, STRING, ARRAY, etc.
...
Input expression:
```
(A == TRUE AND (B == 1 OR C == 2)) AND D == 3
```

Converted to DNF:
```
(A == TRUE AND B == 1 AND D == 3) OR (A == TRUE AND C == 2 AND D == 3)
```

Storage:
- **Clause Group 1**: A==TRUE, B==1, D==3 (3 requirements)
- **Clause Group 2**: A==TRUE, C==2, D==3 (3 requirements)

### Documentation

See [`ingest/README.md`](ingest/README.md) for:
...
Run the demonstration script to see DNF conversion in action:
