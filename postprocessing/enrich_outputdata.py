...
PARAM_PATTERN = re.compile(
    r"(?P<field>[A-Z][A-Z0-9_]*(?:\.[A-Z][A-Z0-9_]*)+)\s*(?P<op>>=|<=|==|!=|>|<)\s*(?P<val>-?\d+(?:\.\d+)?)"
)
UNIT_NUMBER_PATTERN = re.compile(
    r"(?P<val>\d+(?:\.\d+)?)\s?(?P<unit>N|m|kg|personas?|N/m2|kg/m2)\b",
    re.IGNORECASE,
)
...
def build_existing_param_index(
    obj: Dict[str, Any],
  ...
  for norm in norms:
    ...
    for fld in ("applies_if", "satisfied_if", "exempt_if"):
      ...
      for m in PARAM_PATTERN.finditer(expr):
        ...
        unit = (
            None  # unit not in DSL component; may capture from norm text later
        )
    ...
    for mu in UNIT_NUMBER_PATTERN.finditer(text_snip):
      ...
      unit_found: Optional[str] = (
          mu.group("unit").upper() if mu.group("unit") else None
      )
...
def merge_duplicate_tags(obj: Dict[str, Any]):
  ...
  if merged:
    obj.setdefault("quality", {}).setdefault("warnings", []).append(
        f"DUPLICATE_TAGS_MERGED:{merged}"
    )
