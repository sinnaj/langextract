...
def ensure_tag(obj: Dict[str, Any], tag_path: str) -> Optional[str]:
  ...
  obj.setdefault("quality", {}).setdefault("warnings", []).append(
      f"PLACEHOLDER_TAG_CREATED:{tag_path}"
  )
  return new_id
...
def infer_relationships(obj: Dict[str, Any]):
  id_index = {
      ...
      for item in obj.get(k, [])
      if isinstance(item, dict)
  }
  tag_by_path = {
      t.get("tag_path"): t for t in obj.get("tags", []) if isinstance(t, dict)
  }
  ...
  for norm in obj.get("norms", []):
    for fld in ("satisfied_if", "applies_if", "exempt_if"):
      ...
      for m in re.finditer(r"ANNEX\.([A-Z0-9_]+)\.SUBMITTED", expr):
        code = (
            f"Anexo {m.group(1)}"
            if not m.group(1).startswith("Anexo")
            else m.group(1)
        )
...
def compute_extended_metrics(obj: Dict[str, Any]) -> Dict[str, float]:
  ...
  for c in consequences:
    if c.get("source_norm_ids") or c.get("activates_norm_ids"):
      cons_linked += 1
  consequence_linkage_rate = (
      cons_linked / len(consequences) if consequences else 0.0
  )
  ...
  for n in norms:
    ...
    if any(
      ...
      norms_with_codes += 1
  location_scope_population_rate = (
      norms_with_codes / len(norms) if norms else 0.0
  )
  return {
      "question_tag_alignment_rate": round(question_tag_alignment_rate, 3),
      "consequence_linkage_rate": round(consequence_linkage_rate, 3),
      "location_scope_population_rate": round(
          location_scope_population_rate, 3
      ),
  }
