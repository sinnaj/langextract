...
from runner import build_worker_cmd
...
VENDOR_ASSETS: dict[str, str] = {
    ...
    str(
        VENDOR_ROOT / "json-formatter" / "json-formatter.umd.js"
    ): "https://cdn.jsdelivr.net/npm/json-formatter-js@2/dist/json-formatter.umd.js",
    str(
        VENDOR_ROOT / "json-formatter" / "json-formatter.css"
    ): "https://cdn.jsdelivr.net/npm/json-formatter-js@2/dist/json-formatter.css",
    # GitHub Markdown CSS for nicer MD rendering
    str(
        VENDOR_ROOT / "github-markdown.min.css"
...
def _download_url_to_file(
    url: str, dest_path: Path, timeout: float = 10.0
) -> bool:
  ...
  try:
    ...
    with urlopen(
        url, timeout=timeout
    ) as resp:  # nosec - fetching public static assets
      data = resp.read()
...
def start_run():
  ...
  payload = {
      "MODEL_ID": model_id,
      "MODEL_TEMPERATURE": (
          float(model_temperature) if model_temperature else 0.15
      ),
  ...
  (run_dir / "run_input.json").write_text(
      json.dumps(run_input, indent=2), encoding="utf-8"
  )
    ...
    while True:
      try:
        ...
        while idx - offset < len(buf):
          ...
          yield (
              "data:"
              f" {json.dumps({'line': line, 'run_id': run_id, 'ts': time.time()})}\n\n"
          )
          last_emit = time.time()
...
def run_file(run_id: str):
  ...
  try:
    ...
    return abort(404)
  if (
      not str(abs_path).startswith(str(run_dir_res))
  ...
  if preview_flag:
    try:
      ...
      if getattr(oe, "errno", None) in (
          13,
          32,
      ):  # Permission denied / sharing violation
  ...
  try:
    ...
    resp = Response(
        "File is temporarily locked; please retry shortly.",
        status=423,
    ...
    if getattr(oe, "errno", None) in (13, 32):
      resp = Response(
          "File is temporarily unavailable; please retry shortly.",
          status=423,
          mimetype="text/plain; charset=utf-8",
      )
      resp.headers["Retry-After"] = "1"
  ...
  try:
    ...
    if comment.parent_comment_id:
      ...
      if parent.parent_comment_id is not None:
        return (
            jsonify({"error": "Cannot reply to a reply (max depth is 1)"}),
            400,
        )
...
def _acquire_single_instance_lock(host: str, port: int) -> None:
  ...
  if LOCK_FILE_PATH.exists():
    ...
    try:
      ...
      print(
          "Could not remove stale lock file; aborting start.", file=sys.stderr
      )
      sys.exit(1)
  try:
