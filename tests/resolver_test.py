...
class ControlCharacterHandlingTest(parameterized.TestCase):
  """Test cases for improved control character handling in the resolver."""

  def setUp(self):
    """Set up test fixtures."""
    self.resolver_with_suppress = resolver_lib.Resolver(
    ...
    self.resolver_without_suppress = resolver_lib.Resolver(
        format_type=data.FormatType.JSON,
        fence_output=True,
  ...
  def test_early_sanitization_removes_control_characters(self):
    """Test that control characters are removed early in the process."""
...
```'''
    ...
    result = self.resolver_with_suppress.resolve(test_input)
    self.assertEqual(len(result), 2)
    
    # Check that the control character was removed
    text_extraction = next((ext for ext in result if ext.extraction_class == "extraction_text"), None)
    self.assertIsNotNone(text_extraction)
    self.assertNotIn('\x00', text_extraction.extraction_text)
    self.assertIn('Text with control char:', text_extraction.extraction_text)
    ...
    test_input = '''```json
{
  "extractions": [{
    "extraction_class": "Test",
    "extraction_text": "Text with multiple: \x01\x02\x03\x1F chars"
  }]
}
```'''
    ...
    text_extraction = next((ext for ext in result if ext.extraction_class == "extraction_text"), None)
    self.assertIsNotNone(text_extraction)
    self.assertNotIn('\x01', text_extraction.extraction_text)
    self.assertNotIn('\x02', text_extraction.extraction_text)
    self.assertNotIn('\x03', text_extraction.extraction_text)
    self.assertNotIn('\x1F', text_extraction.extraction_text)
...
```'''
    ...
    text_extraction = next((ext for ext in result if ext.extraction_class == "extraction_text"), None)
    self.assertIsNotNone(text_extraction)
    self.assertIn('\t', text_extraction.extraction_text)  # TAB preserved
    self.assertIn('\n', text_extraction.extraction_text)  # LF preserved  
    self.assertIn('\r', text_extraction.extraction_text)  # CR preserved

  def test_suppress_parse_errors_with_control_characters(self):
    """Test suppress_parse_errors behavior with control characters."""
    test_input = '''```json
{
  "extractions": [{
    "extraction_class": "Test",
    "extraction_text": "Text with \x00 control"
  }]
}
```'''
    
    # With suppress_parse_errors=True, should return results
    result_suppressed = self.resolver_with_suppress.resolve(test_input, suppress_parse_errors=True)
    self.assertEqual(len(result_suppressed), 2)
    
    # With suppress_parse_errors=False, should also return results (after sanitization)
    result_not_suppressed = self.resolver_without_suppress.resolve(test_input, suppress_parse_errors=False)
    self.assertEqual(len(result_not_suppressed), 2)

  def test_malformed_json_with_suppress_parse_errors_true(self):
    """Test that malformed JSON returns empty list with suppress_parse_errors=True."""
    malformed_input = '''```json
{
  "extractions": [{
    "extraction_class": "Test",
    "extraction_text": "unclosed string
  }]
}
```'''
    
    result = self.resolver_with_suppress.resolve(malformed_input, suppress_parse_errors=True)
    self.assertEqual(len(result), 0)

  def test_malformed_json_with_suppress_parse_errors_false(self):
    """Test that malformed JSON raises exception with suppress_parse_errors=False."""
    malformed_input = '''```json
{
  "extractions": [{
    "extraction_class": "Test", 
...
```'''
    ...
    with self.assertRaises(resolver_lib.ResolverParsingError):
      self.resolver_without_suppress.resolve(malformed_input, suppress_parse_errors=False)

  def test_large_input_performance(self):
    """Test that large inputs with control characters are handled efficiently."""
