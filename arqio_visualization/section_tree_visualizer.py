...
class SectionTreeBuilder:
  ...
  def _create_node(self, extraction: Dict[str, Any]) -> None:
    ...
    if extraction_class == 'SECTION':
      ...
      parent_id = attributes.get('parent_section_id') or attributes.get(
          'parent_id'
      )
      summary = (
          f"Paragraph {attributes.get('paragraph_number', 'N/A')} -"
          f" {attributes.get('obligation_type', 'Unknown')}"
      )

    elif extraction_class == 'TABLE':
      title = (
          attributes.get('table_title')
          or extraction.get('extraction_text', '')[:50]
      )
      ...
      parent_id = attributes.get('parent_id')
      summary = (
          f"{attributes.get('doc_type', 'Document')} -"
          f" {attributes.get('jurisdiction', 'Unknown jurisdiction')}"
      )
  ...
  def generate_html_visualization(
      self, output_file: str = 'section_tree_visualization.html'
  ) -> None:
...
<body>
  ...
  def _generate_node_html(self, node: SectionNode, level: int) -> str:
    ...
    if node.children:
      collapse_btn = (
          '<div class="collapse-btn" title="Click to expand/collapse"></div>'
      )
    ...
    type_badge = (
        f'<span class="node-type">{node.sectioning_type}</span>'
...
def main():
  ...
  json_file_path = (
      r'c:\Projects\Arqio\LangExtract\langextract\output_runs\1756836143\lx'
      r' output\annotated_extractions_single.json'
  )
